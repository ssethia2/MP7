...
public class Battleship {
	public static void main(String args[]) {
		...
		int[] player_hits = { 0, 0, 0, 0, 0 };
		int[] comp_hits = { 0, 0, 0, 0, 0 };
		int player_win = 0;
		int comp_win = 0;
		for (int i = 1; i < 6; i++) {
			...
			if(i==1)
				comp_ships[i - 1] = new Ships("Computer's ship " + Integer.toString(i), 2);
			else
				comp_ships[i - 1] = new Ships("Computer's ship " + Integer.toString(i), i);
			do {
				x = (int) (Math.random() * 9);
				y = (int) (Math.random() * 9);
		...
		/*for (int i = 0; i < 10; i++) {
			for (int j = 0; j < 10; j++) {
				if (comp_board.getGameBoard()[i][j] == null)
					System.out.print("O\t");
				else
					System.out.print(comp_board.getGameBoard()[i][j].getName()+"\t");
			}
			System.out.println();
		}*/
		System.out.println("\nTime to play!");
		...
		while (true) {
			...
			if (comp_board.checkBoard(x, y) == 0) {
				...
				if (comp_board.checkBoard(x, y) == 1) {
					...
					for (int k = 0; k < 5; k++) {
						if (comp_board.getGameBoard()[x][y].getId() == comp_ships[k].getId()) {
							if (comp_hits[k] == comp_ships[k].getLength()) {
								System.out.println("You sank " + comp_board.getGameBoard()[x][y].getName());
								break;
							} else {
								comp_hits[k]++;
			...
			if (player_board.checkBoard(x, y) == 0) {
				...
				if (player_board.checkBoard(x, y) == 1) {
					...
					// System.out.println("You sank " + comp_board.getGameBoard()[x][y].getName());
					for (int k = 0; k < 5; k++) {
						if (player_board.getGameBoard()[x][y].getId() == player_ships[k].getId()) {
							if (player_hits[k] == player_ships[k].getLength()) {
